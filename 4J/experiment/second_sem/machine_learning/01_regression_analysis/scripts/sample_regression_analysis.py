...
def cul_R(y: np.ndarray, y_pred: np.ndarray):
    mean_y = np.mean(y)
    return(1 - ((((y-y_pred).T)@(y-y_pred))/(((y-mean_y).T)@(y-mean_y))))

def cul_RMSE(y: np.ndarray, y_pred: np.ndarray):
    n = len(y)
    return np.sqrt(((y-y_pred).T @ (y-y_pred)) / n)
...
def main():
    ...
    print(f"R^2 = {cul_R(y, y_pred)}")
    print(f"RMSE = {cul_RMSE(y, y_pred)}")
