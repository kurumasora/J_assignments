...
def gradient_method(x, y, learning_rate, max_iteration):
    ...
    w = rng.standard_normal()# [TASK] 初期の重みをランダムに設定
    b = rng.standard_normal()# [TASK] 初期の重みをランダムに設定
    ...
    for i in range(max_iteration):
        ...
        # 重みとバイアスの勾配を計算
        g_w = 2 * np.mean(x * (fx - y))# [TASK] 重みの勾配を計算
        g_b = 2 * np.mean(fx - y)# [TASK] バイアスの勾配を計算

        # パラメータ更新
        w = w - learning_rate * g_w# [TASK] 重みを更新
        b = b - learning_rate * g_b# [TASK] バイアスを更新

        # 誤差計算
        e = np.mean((fx - y) ** 2) # [TASK] 平均二乗誤差を計算
