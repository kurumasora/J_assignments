...
class Perceptron():
    ...
    def __init__(self, input_size, output_size, learning_rate=0.01):
        ...
        self.w = rng.standard_normal((input_size,output_size))# [TASK] 初期の重みをガウス分布に従う乱数で初期化
        # バイアス(切片)をガウス分布に従う乱数で初期化
        self.b = rng.standard_normal(output_size) # [TASK] 初期のバイアスをガウス分布に従う乱数で初期化
    ...
    def forward(self, x):
        ...
        # 順伝播y = wx + bを計算
        self.y = x @ self.w + self.b# [TASK] 順伝播を計算
    ...
    def backward(self, grad):
        ...
        # 重みとバイアスの勾配を計算
        g_w = self.x.T @ grad# [TASK] 重みの勾配を計算
        g_b = np.sum(grad, axis = 0)# [TASK] バイアスの勾配を計算

        # 逆伝播する勾配を計算
        g =  grad @ self.w.T# [TASK] 逆伝播で入力側に与える勾配情報を計算

        # 重みとバイアスの更新
        self.w = self.w - self.learning_rate* g_w# [TASK] 重みを更新
        self.b = self.b - self.learning_rate * g_b# [TASK] バイアスを更新
