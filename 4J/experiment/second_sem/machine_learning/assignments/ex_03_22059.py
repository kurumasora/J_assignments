...
def plot_losses(train_losses, test_losses, output_dir="../outputs"):
    ...
    out_path = os.path.join(output_dir, "ex_mnist_loss.png")
...
def plot_accuracy(train_accs, test_accs, output_dir="../outputs"):
    ...
    out_path = os.path.join(output_dir, "ex_mnist_accuracy.png")
    plt.savefig(out_path)
    plt.close()
...
class NeuealNetworkCls(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(784, 100)# [TASK][ASSIGNMENT]

        self.relu = nn.ReLU()#[ASSIGNMENT]

        self.fc2 = nn.Linear(100, 10)# [ASSIGNMENT]
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)# [TASK]
        x = self.relu(x)# [ASSIGNMENT]
        x = self.fc2(x)# [ASSIGNMENT]
        return x
